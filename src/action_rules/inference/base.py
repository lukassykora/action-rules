...
class ConfidenceIntervalResult:
    ...
    undefined_bootstrap_fraction : float, optional
        For the bootstrap engine: fraction of resamples that hit a degenerate
        configuration (``n_u_ante == 0`` or ``n_d_ante == 0``) and were therefore
        dropped from the resulting percentile interval.  ``0.0`` is the
        well-behaved case.  Set to ``None`` for analytic / Bayesian engines.
    ...
    undefined_bootstrap_fraction: Optional[float] = None
    ...
    def to_dict(self, include_samples: bool = False) -> dict:
        ...
        include_samples : bool
            When ``False`` (default) the ``samples_uplift`` and ``samples_gain``
            arrays are omitted to keep the dict compact and JSON-friendly.
            When ``True`` the arrays are converted to lists.

        Returns
        -------
        dict
            Keys mirror the dataclass fields.  ``category`` (if present) is
            converted to its ``RuleCategory.value`` string.
        """
        out: dict = {
            'rule_index': self.rule_index,
            'method': self.method,
            'confidence_level': self.confidence_level,
            'uplift_point': self.uplift_point,
            'uplift_ci_lower': self.uplift_ci_lower,
            'uplift_ci_upper': self.uplift_ci_upper,
            'uplift_se': self.uplift_se,
            'realistic_rule_gain_point': self.realistic_rule_gain_point,
            'realistic_rule_gain_ci_lower': self.realistic_rule_gain_ci_lower,
            'realistic_rule_gain_ci_upper': self.realistic_rule_gain_ci_upper,
            'realistic_rule_gain_se': self.realistic_rule_gain_se,
            'support': self.support,
            'confidence': self.confidence,
            'category': self.category.value if self.category is not None else None,
            'undefined_bootstrap_fraction': self.undefined_bootstrap_fraction,
        }
        if include_samples:
            out['samples_uplift'] = None if self.samples_uplift is None else self.samples_uplift.tolist()
            out['samples_gain'] = None if self.samples_gain is None else self.samples_gain.tolist()
        return out
...
def results_to_dataframe(
    ...
    """Convert a list of :class:`ConfidenceIntervalResult` to a tidy DataFrame.

    One row per result, columns mirror :meth:`ConfidenceIntervalResult.to_dict`.
    Sample arrays are omitted by default to keep the frame compact.

    Parameters
    ----------
    results : list of ConfidenceIntervalResult
    include_samples : bool
        When ``True``, include ``samples_uplift`` and ``samples_gain`` as
        object columns (each cell is a list of floats).  Default ``False``.

    Returns
    -------
    pd.DataFrame
        Empty DataFrame when *results* is empty.
    """
    if not results:
        return pd.DataFrame()
    return pd.DataFrame([r.to_dict(include_samples=include_samples) for r in results])
